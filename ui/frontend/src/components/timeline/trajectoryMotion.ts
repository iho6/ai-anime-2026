...
export function trajectoryTransformAt(
  ...
  const holdT = holdTEnd(a, aT, bT, dur);

  if (t < holdT) {

    return waypointPose(a);

  }

  if (holdT >= bT) {

    return waypointPose(b);

  }



  let s = linearT(t, holdT, bT);



  const pauseAtA = holdT > aT + 1e-9;
  ...
  const next = i + 2 < wps.length ? wps[i + 2] : null;

  const nextT = next?.t ?? bT;

  const pauseAtB = effectiveHoldSec(b, bT, nextT, dur) > 0;

  // Final waypoint: allow arrival glide even without a following pause segment.

  const arriveAtFinal = next == null && normalizeBlendEase(b.blendEase) > 0;
