/**
 * Framework-agnostic Three.js camera gizmo, ported verbatim from
 * jtydhr88/ComfyUI-qwenmultiangle (src/CameraWidget.ts). Drives an azimuth ring,
 * elevation arc and distance handle around a subject card; emits CameraState on change.
 *
 * `three` is imported here; this module is only loaded lazily by CameraAngleModal, so the
 * three.js bundle is not shipped until the user opens the angle gizmo.
 */
import * as THREE from "three";
import type { CameraState, CameraWidgetOptions } from "./types";

export type { CameraState, CameraWidgetOptions } from "./types";
...
export class CameraWidget {
  ...
  private updateDistanceLine(start: THREE.Vector3, end: THREE.Vector3): void {
    ...
    if (this.useCameraView) this.distanceTube.visible = false;
