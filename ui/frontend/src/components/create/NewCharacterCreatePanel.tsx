...
export type NewCharacterCreatePanelHandle = {
  hasDrafts: () => boolean;
  cancelWithConfirm: () => Promise<boolean>;
  /** Confirm and discard drafts when closing the parent modal (does not call onCancelled). */
  discardDraftsWithConfirm: () => Promise<boolean>;
};

export type NewCharacterCreatePanelProps = {
  variant: "page" | "embedded";
  onFinalized: (charKey: string) => void;
  /** Called after cancel is confirmed and draft workspace is cleared (or was empty). */
  onCancelled: () => void;
  cancelConfirmMessage?: string;
};

export const NewCharacterCreatePanel = forwardRef<
  NewCharacterCreatePanelHandle,
  NewCharacterCreatePanelProps
>(function NewCharacterCreatePanel(props, ref) {
  const {
    variant,
    onFinalized,
    onCancelled,
